...
public class StudentController extends UserController{
    ...
    public StudentDataModel loginStudent(@RequestBody StudentDataModel s){
        ...
        try (Connection connection = dataSource.getConnection()) {
            Statement stmt = connection.createStatement();
            PreparedStatement preparedStatement = connection.prepareStatement("SELECT userId, username, passhash FROM users WHERE userName = ? AND passhash = crypt(?, gen_salt('bf'))");
